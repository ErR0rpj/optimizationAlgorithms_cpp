// This problem solves the complex linear programming problem. Strigler diet problem is
...
const std::vector<std::pair<std::string, double>> nutrients = {
    {"Calories (kcal)", 3.0}, {"Protein (g)", 70.0}, {"Calcium (g)", 0.8}, {"Iron (mg)", 12.0}, {"Vitamin A (kIU)", 5.0}, {"Vitamin B1 (mg)", 1.8}, {"Vitamin B2 (mg)", 2.7}, {"Niacin (mg)", 18.0}, {"Vitamin C (mg)", 75.0}};
...
struct FoodItem
{
    ...
    std::string unit;
    double price;
    std::array<double, 9> nutrients; // per cent
};

std::vector<FoodItem> foodData = {
...
namespace operations_research
{
    void LinearSolution()
    {
        ...
        // Defines the variables for
        std::vector<MPVariable *> foods;
        const double infinity = solver->infinity();

        for (const FoodItem &item : foodData)
        {
            foods.push_back(solver->MakeNumVar(1, infinity, item.name));
        }
        LOG(INFO) << "Number of foodItems = " << solver->NumVariables();

        // Creating constraints
        std::vector<MPConstraint *> constraints;
        for (int i = 0; i < nutrients.size(); i++)
        {
            constraints.push_back(solver->MakeRowConstraint(nutrients[i].second, infinity));
            for (int j = 0; j < foodData.size(); j++)
            {
                constraints.back()->SetCoefficient(foods[j], foodData[j].nutrients[i]);
            }
        }
        LOG(INFO) << "Number of constraints/Nutrients = " << constraints.size();
...
int main()
{
    operations_research::LinearSolution();
    return EXIT_SUCCESS;
}
